                    ...
                    <table class="table table-striped" id="myTable">
                        <thead>
                            <tr>
                                ...
                                <th class="bg-secondary text-white">Partners</th>
                                <th class="bg-secondary text-white">Topics</th>
                                <th class="bg-secondary text-white">Status</th>
                                <th class="bg-secondary text-white">Featured Image</th>
                                @can(['view-admin-project', 'edit-admin-project', 'delete-admin-project'])
        ...
        async function getList() {
            ...
            try {
                ...
                res.data.data.forEach(function (item, index) {
                    let partners = item.partners.map(partner => `<span class="badge bg-primary text-light">${partner.name}</span>`).join(' ');
                    let topics = item.topics.map(topic => `<span class="badge bg-success text-light">${topic.name}</span>`).join(', ');
                    let row = `<tr>
                                    ...
                                    <td>${topics}</td>
                                    <td>${item.status}</td>
                                    <td>${item.featured_image ? `<img src="${item.featured_image}" alt="Featured Image" style="width: 80px; height: auto;">` : 'No Featured Image'}</td>
                                    @can(['view-admin-project', 'edit-admin-project', 'delete-admin-project'])
                                        <td class="action">
